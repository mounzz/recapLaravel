...
<div style="display:flex;gap: 1rem; margin: 1rem 0;border: 1px solid silver;padding: 1rem;">
    <div>{{$batiment -> nom}}</div>
    <div> {{$batiment -> description}}</div>
    <div>
        <a href="/batiments/{{$batiment->id}}">
        <button type="submit">
            show
        </button>
        </a>
    ...
    <div>
        <form style="display:inline;" action="/batiments/{{$batiment->id}}/delete" method="POST">
            @csrf
            @method('DELETE')
...
<form action="/batiments/deleteAll" method="POST">
    @csrf
